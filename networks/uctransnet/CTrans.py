...
class Channel_Embeddings(nn.Module):
    ...
    """
    def __init__(self,config, patchsize, img_size, in_cannels):
        super().__init__()
        img_size = _pair(img_size)
        patch_size = _pair(patchsize)
        n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])

        self.patch_embeddings = Conv2d(in_cannels=in_cannels,
                                       num_classannels=in_cannels,
                                       kernel_size=patch_size,
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, in_cannels))
...
class Reconstruct(nn.Module):
    def __init__(self, in_cannels, num_classannels, kernel_size, scale_factor):
        super(Reconstruct, self).__init__()
        if kernel_size == 3:
            padding = 1
        else:
            padding = 0
        self.conv = nn.Conv2d(in_cannels, num_classannels,kernel_size=kernel_size, padding=padding)
...
class Mlp(nn.Module):
    def __init__(self,config, in_cannel, mlp_channel):
        super(Mlp, self).__init__()
        self.fc1 = nn.Linear(in_cannel, mlp_channel)
        self.fc2 = nn.Linear(mlp_channel, in_cannel)
        self.act_fn = nn.GELU()
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        self.embeddings_1 = Channel_Embeddings(config,self.patchSize_1, img_size=img_size,    in_cannels=channel_num[0])
        self.embeddings_2 = Channel_Embeddings(config,self.patchSize_2, img_size=img_size//2, in_cannels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(config,self.patchSize_3, img_size=img_size//4, in_cannels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(config,self.patchSize_4, img_size=img_size//8, in_cannels=channel_num[3])

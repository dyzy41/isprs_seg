...
from networks.uctransnet import uctrans_config as config_inside

config_vit = config_inside.get_CTranS_config()
...
class UpBlock_attention(nn.Module):
    def __init__(self, in_cannels, num_classannels, nb_Conv, activation='ReLU'):
        ...
        self.coatt = CCA(F_g=in_cannels // 2, F_x=in_cannels // 2)
        self.nConvs = _make_nConv(in_cannels, num_classannels, nb_Conv, activation)
...
class UCTransNet(nn.Module):
    def __init__(self, config=config_vit, n_channels=3, num_class=1, img_size=512, vis=False):
        ...
        self.mtc = ChannelTransformer(config, vis, img_size,
                                      channel_num=[in_cannels, in_cannels * 2, in_cannels * 4, in_cannels * 8],
                                      patchSize=config.patch_sizes)
        self.up4 = UpBlock_attention(in_cannels * 16, in_cannels * 4, nb_Conv=2)
        self.up3 = UpBlock_attention(in_cannels * 8, in_cannels * 2, nb_Conv=2)
        self.up2 = UpBlock_attention(in_cannels * 4, in_cannels, nb_Conv=2)
        self.up1 = UpBlock_attention(in_cannels * 2, in_cannels, nb_Conv=2)
        self.outc = nn.Conv2d(in_cannels, num_class, kernel_size=(1, 1), stride=(1, 1))

...
class BlockAttention(nn.Module):
    def __init__(self, in_c=256, block_size=1):
    ...
    def forward(self, x):
        length = x.size()[-1]
        x_Q = self.conv_Q(x)
        x_Q = rearrange(x_Q, 'b c (hs bs) w -> b c hs (w bs)', hs=length // self.block_size, bs=self.block_size)

        x_K = self.conv_K(x)
        x_K = rearrange(x_K, 'b c (hs bs) w -> b c hs (w bs)', hs=length // self.block_size, bs=self.block_size)
        x_K = x_K.transpose(3, 2)

        att_map = torch.matmul(x_Q, x_K)
        att_map = self.softmax(att_map)
        att_map = F.interpolate(att_map, scale_factor=self.block_size, mode='nearest')
...
class MANet(nn.Module):
    def __init__(self, in_c, num_class, bilinear=True):
        ...
        self.backbone = get_model('hrnet_w64')
        self.inc = DoubleConv(in_c, 64)

        self.conv_block2 = DoubleConv(128, 256)
        self.conv_block3 = DoubleConv(256, 256)
        self.conv_block4 = DoubleConv(512, 256)
        self.conv_block5 = DoubleConv(1024, 256)

        self.att2 = BlockAttention(block_size=8)
        self.att3 = BlockAttention(block_size=4)
        self.att4 = BlockAttention(block_size=2)
        self.att5 = BlockAttention(block_size=1)

        self.base_conv = DoubleConv(256, 256)
        self.base_conv2 = DoubleConv(256*4, 256)
    ...
    def forward(self, x):
        ...
        x4 = self.base_conv(x4)
        x5 = self.base_conv(x5)

        x3 = F.interpolate(x3, size=(x2.size()[2], x2.size()[3]), mode='bilinear', align_corners=True)
        x4 = F.interpolate(x4, size=(x2.size()[2], x2.size()[3]), mode='bilinear', align_corners=True)
        x5 = F.interpolate(x5, size=(x2.size()[2], x2.size()[3]), mode='bilinear', align_corners=True)

        out = torch.cat([x2, x3, x4, x5], dim=1)
        out = self.base_conv2(out)
